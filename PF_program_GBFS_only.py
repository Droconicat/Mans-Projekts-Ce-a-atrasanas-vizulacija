...
Algo_Text = tk.Label(TopFrame, text = "Izvēlētā algoritms:", bg="grey50", fg="white", width=15, height=1, font=("Arial", 15))
Algo_Text.place(in_ = TopFrame, relx = 0.46, rely = 0.4 ,anchor = "se")
...
def Greedy_Best_First_Search():
    ...
    while len(Open) != 0:
        ...
        Current_Node.Button.configure(bg="blue4")
        ...
        for node in Neighbors:
            ...
            if not node in Open:
                ...
                node.Button.configure(bg="cyan4")
                Color_Changed_nodes.append(node)
                Open.append(node)
...
def Grid_Button_Pressed(x, y):
    ...
    if node.Type == 0:
        ...
        node.Type = 0
        node.Button.configure(bg="grey75")
...
def Start_Button_Pressed():    
    ...
    global Button_Bool
    global Grid
    global Color_Changed_nodes
    
    if not Working:
        Working = True
        Button_SPF.configure(text = "Meklējam...", bg = "yellow", fg = "black")
        Path = Greedy_Best_First_Search()
        sleep(0.5)
        
        prev_node = Path[len(Path)-1]
        
        Start.Button.configure(bg="green")
        
        while prev_node != Start:
            if prev_node.Type != 3:
                prev_node.Button.configure(bg = "yellow")
            else:    
                prev_node.Button.configure(bg = "red")
            
            prev_node = prev_node.Parent
        
        Button_SPF.configure(text = "Iztīrīt", bg = "orange", fg = "white")
        Button_Bool = True
        return 0
    ...
    if Button_Bool:
        Button_Bool = False
        Button_SPF.configure(text = "Tīram...", bg = "yellow", fg = "black")
        
        for node in Color_Changed_nodes:
            node.Button.configure(bg = color[node.Type])
            sleep(0.05)
        
        Color_Changed_nodes.clear()
        
        Button_SPF.configure(text = "Meklēt!", bg = "orange", fg = "white")
        Working = False
        return 0
...
for X in range(0, 32):
    Grid.append([])
    for Y in range(0, 28):
        New_Button = tk.Button(App_Window, text="", width=1, height=1
        , bg="grey75", command = lambda arg1=X, arg2=Y: Grid_Button_Pressed(arg1, arg2))
        New_Button.place(in_ = App_Window, x = 25*X, y = 100 + (25*Y),anchor = "nw")
        TYPE = 0
        if X == 31 and Y == 0:
            TYPE = 3
